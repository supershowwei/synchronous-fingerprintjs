...
var FingerprintJS = (function (exports) {
    ...
    ***************************************************************************** */
    ...
    function getSync() {
        const includedSources = Object.keys(sources).filter(function (sourceKey) { return excludes(["fonts", "domBlockers", "fontPreferences", "audio", "screenFrame"], sourceKey); });
        const components = {};

        for (let index = 0; index < includedSources.length; index++) {
            const sourceKey = includedSources[index];
            const loadStartTime = Date.now();

            let loadArgs;

            try {
                loadArgs = [true, sources[sourceKey]()];
            } catch (error) {
                loadArgs = [false, error];
            }

            const loadDuration = Date.now() - loadStartTime;

            if (loadArgs[0]) {
                components[sourceKey] = { value: loadArgs[1], duration: loadDuration };
            } else {
                components[sourceKey] = { error: ensureErrorWithMessage(loadArgs[1]), duration: loadDuration };
            }
        }

        let visitorIdCache;
        let confidence = getConfidence(components);
